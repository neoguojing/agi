...
class TestTaskMultiModalFactory(unittest.TestCase):
    ...
    def test_text2speech_chain(self):
        ...
        self.assertIsNotNone(resp.content[1].get("audio"))
        self.assertEqual(resp.content[1].get("type"),"audio")
    ...
    def test_text2image_chain(self):
        ...
        self.assertIsInstance(resp,AIMessage)
        self.assertIsInstance(resp.content,list)
        self.assertIsNotNone(resp.content[1].get("image"))
        self.assertEqual(resp.content[1].get("type"),"image")
        # self.assertIsNotNone(resp.content)
        resp = llm_task.invoke({"text":"猫咪在游泳","data":"tests/cat.jpg","type":"image"})
        self.assertIsInstance(resp,AIMessage)
        self.assertIsInstance(resp.content,list)
        self.assertIsNotNone(resp.content[1].get("image"))
        self.assertEqual(resp.content[1].get("type"),"image")
