...
class TestTextToSpeechCosyVoice(unittest.TestCase):
...
hay: Haya — 指的是坦桑尼亚的一种语言，由Haya人使用，属于尼日尔-刚果语系。
    ...
    def test_text2speech_stream(self):
        config={"configurable": {"user_id": "cosyvoice", "model": "cosyvoice"}}
        output = self.instance.stream(self.input,config=config)
        print(output)
        for ai_msg in output:
            self.assertIsInstance(ai_msg,AIMessage)
            for item in ai_msg.content:
                context_type = item.get("type") 
                if context_type == "audio" and item.get("audio"):
                    decoded_chunk = base64.b64decode(item.get("audio"))
                    print(f"base64:{len(item.get('audio'))},pcm:{len(decoded_chunk)}")
                else:
                    print(ai_msg)
