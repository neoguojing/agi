...
from agi.utils.yacy import YaCySearch  # 假设函数在your_module.py里

YACY_HOST = "http://localhost:8090"

client = YaCySearch(max_results=5)

def test_yacy_search_real_call():
...
#     search_result = await client.yacy_search_async(query=query, maximum_records=max_records)
...
#         # 调用爬虫启动函数，真实调用
#         await client.start_yacy_crawl_async(url, username="admin", password="yacy_password")
