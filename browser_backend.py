...
class StatefulBrowserBackend(AbstractBrowserBackend):
    ...
    def _update_page_runtime_state(self, page: Page, *, load_state: str | None = None) -> None:
        ...
        # 如果 PageInfo 不存在，创建一个新的；否则更新现有 PageInfo
        ...
        state.url = page.url
        state.title = load_state or state.title
        ...
        *,
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                # 更新 PageInfo 状态
                page_info = await self._capture_page_info(
                    page,
                    capture_url or page.url,
                    response,
                    capture_content=False,
                )
                state = self._page_runtime_state.get(self._page_id(page))
                if state:
                    state.title = page_info.title
                    state.html = page_info.html
                    state.text = page_info.text
                    state.screenshot_path = page_info.screenshot_path
                    state.metadata.update(page_info.metadata)
    ...
    def get_state_snapshot(
        ...
        *,
        ...
        # 如果当前没有 PageInfo，使用 last_result 或 fallback
        if current_page_state is None and last_result:
            current_page_state = last_result
        ...
        # 构建当前页面状态，优先使用 PageInfo 中的完整数据
        if current_page_state and isinstance(current_page_state, PageInfo):
            current_page_dict = {
                "url": current_page_state.url or "",
                "title": current_page_state.title,
                "load_state": current_page_state.metadata.get("status", "unknown"),
            }
        elif current_page_state:
            current_page_dict = self._page_summary(current_page_state)
        else:
            current_page_dict = {
                "url": "",
                "title": None,
        ...
        snapshot: BrowserSessionSnapshot = {
            ...
            "current_page": current_page_dict,
