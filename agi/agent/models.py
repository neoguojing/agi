...
class DynamicModelRouter:
    ...
    def _create_chat_instance(self, node: ModelNode) -> BaseChatModel:
        ...
        if p == "openai":
            ...
            return ChatGoogleGenerativeAI(model=node.model_name,**params)
            
        elif p == "openrouter":
            # OpenRouter 使用 OpenAI 协议标准，通过特殊注入 base_url 兼容
            base_url = params.pop("openai_api_base", "https://openrouter.ai/api/v1")
            return ChatOpenAI(model=node.model_name, openai_api_base=base_url, **params)
