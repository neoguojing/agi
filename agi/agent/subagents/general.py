...
def make_backend(runtime):
    ...
    user_id = None
    session_id = None
    if hasattr(runtime, 'context') and hasattr(runtime.context, 'user_id'):
        user_id = getattr(runtime.context, 'user_id', "admin")
        session_id = getattr(runtime.context, 'thread_id', "default_session")
    else:
        # 降级从 runtime 直接获取
        user_id = getattr(runtime, 'user_id', "admin")
        session_id = getattr(runtime, 'thread_id', "default_session")
