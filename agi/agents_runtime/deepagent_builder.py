...
from .subagents import build_default_subagents
from .system_tools import docker_build_toolchain, execute_shell, run_in_sandbox
...
def _build_store_and_checkpointer(config: AgentRuntimeConfig, backend_requires_store: bool) -> dict[str, Any]:
    extra: dict[str, Any] = {}
    if not backend_requires_store:
        return extra
    ...
    if config.auto_create_store_for_dev:
        try:
...
def build_deep_agent(config: AgentRuntimeConfig):
    ...
    return dep["create_deep_agent"](
        ...
        debug=config.debug,
        name="agi-main-agent",
        **_build_store_and_checkpointer(config, backend_result.requires_store),
    )
